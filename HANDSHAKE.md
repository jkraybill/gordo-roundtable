...
- **Adopter contract.** Downstream Tier 2 / Tier 3 / external adopters consuming panel-protocol inherit SPEC interface contract (round-mechanics + brief shape + finding shape + disposition vocabulary) — multiple instantiations are admissible per SPEC §11.
- **Ratification venue.** Bilateral substance-MCAP records for panel-protocol substance currently live at backchannel `ratification/` (record-008 admission; record-009 SPEC v0.1) per the umbrella's hub-convention pre-RC1 (backchannel S18). panel-protocol's own ratification/ directory will emerge through use if and when it warrants — not pre-codified at first-substantive-session.
...
3. Confirm current working directory is the panel-protocol repo. `git remote -v` should show `git@github.com:jkraybill/panel-protocol.git`; `pwd` should match `/home/jk/panel-protocol/`.
4. Confirm the environment matches JK's habitual collaboration infrastructure. Home directory at `/home/jk/` with multiple sibling collaborative projects present (`project-gordo`, `project-gordo-backchannel`, `gordo-framework`, `mcap-protocol`, `pact-protocol`, `panel-protocol`); shell and toolchain configured as expected. Environment divergence is a signal to halt and flag rather than proceed. Confirm the human party is JK, not another human operating in similar infrastructure.
...
BOS/EOS patterns for this T1 repo are deferred to first standalone panel-protocol session — to date all panel-protocol work has happened during backchannel hub-sessions per the umbrella's pre-RC1 hub-convention (backchannel `CLAUDE.md` § Session Hub). Cross-repo commits land at panel-protocol; session narrative lives at backchannel `SESSION_LOG.md`. Patterns will emerge if/when panel-protocol has work that doesn't trace to a backchannel hub-session, then codify per the umbrella's emerge-then-codify tradition.
